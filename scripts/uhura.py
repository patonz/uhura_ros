...
def handle_send_string_data(req):
    ...
    if run_type == "simulation":
        for uav in list_names:
            if uav == uav_name:
                continue
            tmp = rospy.Publisher('/%s%s/message_received' % (uav,'/uhuranode'), String, queue_size=10)
...
def uhura_server():
    ...
    rospy.Service('%s/setup_network_device' % (node_name), SetupNetworkDevice, setup)
    rospy.Service('%s/test_broadcast_network' % (node_name), TestBroadcastNetwork, handle_network_test)
    global generic_msg_rcv_pub, position_msg_rcv_pub
    generic_msg_rcv_pub = rospy.Publisher('%s/message_received' % (node_name), String, queue_size=10)
    position_msg_rcv_pub = rospy.Publisher('%s/position_message_received' % (node_name), Position, queue_size=10)
    print("Uhura started")
